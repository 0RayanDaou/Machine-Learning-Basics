...
def gaussian_dataset(n_train=100, n_test=100, s=1.8, std_dev=1.0):
    ...
    def gen_data(n):
        cls1 = np.random.normal((0, 0), (std_dev, std_dev), size=(int(n/2), 2))
        cls2 = np.random.normal((s, s), (std_dev, std_dev), size=(int(n/2), 2))

        x = np.concatenate((cls1, cls2), axis=0)
        y = np.concatenate((np.ones((int(n/2),)), -np.ones((int(n/2),))), axis=0)
